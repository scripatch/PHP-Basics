...
const settings = {
    ...
    speed: 2,
    winFoodCount: 50,
    transparentAllow: true,

    getSettings() {
        const speedValue = document.getElementById('snakeSpeed').value;
        const winCount = document.getElementById('winCount').value;
        const transparentAllow = document.getElementById('transparentAllow').checked;
        return {speed: speedValue, winFoodCount: winCount, transparentAllow: transparentAllow}
    },

    hideSettings() {
        document.querySelector('.settingsWrapper').classList.add('hide');
        document.getElementById('settingsHint').classList.remove('hide');
    },

    showSettings() {
        document.querySelector('.settingsWrapper').classList.remove('hide');
        document.getElementById('settingsHint').classList.add('hide');
    },
};
...
const config = {
    ...
    init() {
        ...
        Object.assign(this.settings, this.settings.getSettings());
    },
    ...
    isTransparentAllow() {
        return this.settings.transparentAllow;
    },
...
/**
 * Объект счётчика игры
 * @property {int} count Текущий счёт игры
 * @property {Element} scoreHolder Элемент HTML, выводящий текущий счет
 */
const score = {
    count: 0,

    scoreHolder: document.getElementById('scoreDisplay'),
    ...
    /**
     * Переносит значение this.count в элемент scoreHolder для отображения счёта
     */
    render() {
        this.scoreHolder.textContent = this.count;
    },
...
const snake = {
        ...
        getNextStepHeadPoint() {
            ...
            if (this.config.isTransparentAllow()) {
                if (newX < 0) {
                    newX = this.config.getColsCount() - 1;
                }
                if (newX > this.config.getColsCount() - 1) {
                    newX = 0;
                }
                if (newY < 0) {
                    newY = this.config.getRowsCount() - 1;
                }
                if (newY > this.config.getRowsCount() - 1) {
                    newY = 0;
                }
            }
 ...
 * @property {boolean} started была ли запущена игра
 */
const status = {
    condition: null,
    started: false,
    ...
    /**
     * Проверяет была ли запущена игра
     * @returns {boolean} true, если игра была запущена, иначе false.
     */
    isStarted() {
        return this.started;
    },
...
const game = {
    ...
    reset() {
        ...
        // Переопрашиваем настройки
        this.config.init();
        // Обнуляем счёт игры
        this.score.reset();
        // Возвращаем настройки
        this.config.showSettings();
        // Игра не стратовала ещё
        this.status.setStartedFalse();
    ...
    playClickHandler() {
        ...
        if (this.status.isPlaying()) {
            ...
            this.play();
            // если игра ещё не была запущена, то опрашиваем настройки и прячем их
    ...
    canMakeStep() {
        ...
        // если стены прозрачные, то проверка только на саму змейку, если нет - то проверяем границы поля и змейку
        if (this.config.isTransparentAllow()) {
